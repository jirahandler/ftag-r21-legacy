...
void ScanDir (string file1, TChain* fileChain, string sysName="") {
  TList* hList = new TList();
  //cout << "Input is a directory: going fancy: " << endl;
  TString SysName = sysName;
  TString path = file1+"/"+SysName+"/";
  cout << "Input path: "<< path << endl;

  char* dir = gSystem->ExpandPathName(path.Data());
  void* dirpath = gSystem->OpenDirectory(dir);
  cout << "About to Loop over Dir Path: " << dir << endl;
  while (const char* file=gSystem->GetDirEntry(dirpath)) {
    // Fist-layer, open the subdir with systName
    if (TString(file)==".") continue;
    if (TString(file)=="..") continue;
    TString DPath = dir;
    //cout << "Print Dir Name: " << DPath << endl;
    TString filename = DPath+"/"+file;
    fileChain->Add(filename);
    //cout << "Print FileName: " << filename << endl;
  } //loop over second directory
  gSystem->FreeDirectory(dirpath);
}
...
void effPlot_eos(string outFileID = "effPlot_FixedCutBEff_85_PFlow-ttbar-full-syst-test", const double bTagCut = 0.11,
                        string filesDir =  "root://eosatlas.cern.ch//eos/atlas/atlascerngroupdisk/perf-flavtag/calib/adjMC/PFlow/FTAG1-ttbar/",
  ...
  TChain stdFile("bTag_AntiKt4EMPFlowJets");
  ...
  double ptbins[] = {20, 60, 100, 200, 300, 500, 750, 1000, 3000}; //to be used for both 1D and 2D
  //changed to match the neg-tag binning
  ...
  double mv2c10_bins[] = {-1.0, 0.11, 0.64, 0.83, 0.94, 1};//New FixedCut WP for R21: No cut, 85%,77%,70% and 60%
  ...
  TH1D *ptFine_h   = new TH1D("ptFine",  "ptFine;jet p_{T} [GeV]", 3000, 0, 3000);//Changed the range to match the Rebin Pt
  ...
  if(fileToMergeInput == "") { //Skip all and just go to the merging phase
    ...
    for(uint i=0; i< trk_sys_v.size(); ++i){
      ...
      TChain tmpFile("bTag_AntiKt4EMPFlowJets");
