...
OUT=${SCOPE}.${HEAD}${DSID}.${LONGPROC}.$((1000 + RANDOM % 9999)).${FLAV}.${TAGS}.${GITT}${APP}
...
if too_long ; then
    OUT=${SCOPE}.${DSID}.$((1000 + RANDOM % 9999)).${PROC}.${FLAV}.${TAGS}.${GITT}${APP}
fi
if too_long ; then
    OUT=${SCOPE}.${DSID}.$((1000 + RANDOM % 9999)).${FLAV}.${TAGS}.${GITT}${APP}
fi
if too_long ; then
