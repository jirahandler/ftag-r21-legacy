...
namespace InDet {
  ...
  static const CP::SystematicSet SmearingSystematics =
    {
      ...
      InDet::TrackSystematicMap[TRK_RES_D0_MEAS_DiJetMap],
      InDet::TrackSystematicMap[TRK_RES_Z0_MEAS_DiJetMap]
  ...
  float InDetTrackSmearingTool::GetSmearD0Sigma(const xAOD::TrackParticle& track) {
    ...
    bool isActiveD0Z0Corl = isActive(TRK_RES_D0Z0Corl_MEAS);
    bool isActiveD0Z0 = isActive(TRK_RES_D0Z0_MEAS);
    bool isActiveD0DiJetMap = isActive(TRK_RES_D0_MEAS_DiJetMap);

    if (isActiveD0Meas + isActiveD0MeasUp + isActiveD0MeasDown + isActiveD0Z0Corl + isActiveD0Z0 + isActiveD0DiJetMap > 1) {
    ...
    } else if (isActiveD0Meas + isActiveD0MeasUp + isActiveD0MeasDown + isActiveD0Z0Corl + isActiveD0Z0 + isActiveD0DiJetMap == 0) {
      // pass-through D0Meas, return sigma_D0 early
      return std::sqrt(sigma_D0);
    }

    if (isActiveD0DiJetMap == 1){
      m_useDijetMaps = true;
      ATH_MSG_INFO( "Running with D0 DiJet Map..." );
    }

    TH2* d0hist_lowpt = nullptr;
    TH2* d0hist_highpt = nullptr;

    if((isActiveD0Meas) || (isActiveD0Z0Corl) || (isActiveD0Z0)||(isActiveD0DiJetMap)){
    ...
    sigma_D0 += d0Smear*d0Smear;
    ATH_MSG_INFO( "Track pT:" << track.pt() << ", after convert to GeV: " << pt);
    ATH_MSG_INFO( "Track eta:" << eta );
    ATH_MSG_INFO( "Smearing in D0:" << sigma_D0 );
  ...
  float InDetTrackSmearingTool::GetSmearZ0Sigma(const xAOD::TrackParticle& track) {
    ...
    bool isActiveD0Z0Corl = isActive(TRK_RES_D0Z0Corl_MEAS);
    bool isActiveD0Z0 = isActive(TRK_RES_D0Z0_MEAS);
    bool isActiveZ0DiJetMap = isActive(TRK_RES_Z0_MEAS_DiJetMap);

    if (isActiveZ0Meas + isActiveZ0MeasUp + isActiveZ0MeasDown + isActiveD0Z0Corl + isActiveD0Z0 + isActiveZ0DiJetMap > 1) {
      // this should be checked against in applySystematicVariation
      throw std::runtime_error( "Multiple incompatible Z0 systematics are activated." );
    } else if (isActiveZ0Meas + isActiveZ0MeasUp + isActiveZ0MeasDown + isActiveD0Z0Corl + isActiveD0Z0 + isActiveZ0DiJetMap == 0) {
      // pass-through Z0Meas, return sigma_Z0 early
      return std::sqrt(sigma_Z0);
    }

    if (isActiveZ0DiJetMap == 1){
      m_useDijetMaps = true;
      ATH_MSG_INFO( "Running with Z0 DiJet Map..." );
    }

    TH2* z0hist_lowpt = nullptr;
    TH2* z0hist_highpt = nullptr;

    if((isActiveZ0Meas) || (isActiveD0Z0Corl) || (isActiveD0Z0)||(isActiveZ0DiJetMap)){
    ...
    ATH_MSG_INFO( "Track pT:" << track.pt() << ", after convert to GeV: " << pt);
    ATH_MSG_INFO( "Track eta:" << eta );
    ATH_MSG_INFO( "Smearing in Z0:" << sigma_Z0 );
  ...
  CP::CorrectionCode InDetTrackSmearingTool::applyCorrection( xAOD::TrackParticle& track ) {
    ...
    if(isActive(TRK_RES_D0Z0Corl_MEAS)){ // then use the same random number to smear both d0 and z0
