...
//std::cout << "A.X.: in assignProbability for " << assigned_jet_author << " " << m_calibrationDirectory << ": " << inputs.size() << " inputs" << std::endl;
    // A.X.: pick one DL1 flavor (DL1, DL1mu, DL1rnn)
    if (m_calibrationDirectory=="DL1") {

      static bool first = true;
      std::ofstream* ffout = 0;
      if (first) ffout = new std::ofstream("dl1inputs.txt");

      TTree* temptree = (TTree*)gDirectory->GetList()->FindObject("temptree");
      static std::vector<double>* dl1inputs = new std::vector<double>();
      if (temptree==0) {
//std::cout << "A.X.: temptree does not exist, creating it." << std::endl;
	temptree = new TTree("temptree","");
	temptree->Branch("dl1inputs",&dl1inputs);
	dl1inputs->clear();
      }
      for (const auto& var: inputs) {
	//std::cout << var.first << " " << var.second << std::endl;
	if (ffout) (*ffout) << var.first << std::endl;
	double myvar = var.second; //if (isnan(myvar)) myvar = -999.;
	dl1inputs->push_back(myvar);
      }
//std::cout << "A.X.: dl1inputs size = " << dl1inputs->size() << std::endl;
      temptree->Fill();
      dl1inputs->clear();

      if (ffout) { ffout->close(); ffout = 0; }
